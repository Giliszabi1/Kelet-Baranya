...
class UserService {
    ...
    async login({username, email, password, client}) {
        ...
        if(user.success == 0){
            if(user.message == "LOGIN_IDENTIFIRY_CANNOT_BE_EMPTY"){
                await logService.create({
                    ...
                    action: "USER_LOGIN",
                    status: "fail",
                    description: "LOGIN_IDENTIFIRY_CANNOT_BE_EMPTY",
                    entity_type: "user",
                    entity_id: null,
                    old_data: null,
                    new_data: JSON.stringify({
        ...
        if(password_verification){
            ...
            await logService.create({
                ...
                action: "USER_LOGIN",
                status: "success",
                description: "USER_LOGIN_SUCCESSFUL",
                entity_type: "user",
                entity_id: user.id,
                old_data: null,
                new_data: null,
    ...
    async logout(token, client) {
        ...
        await logService.create({
            ...
            action: "USER_LOGOUT",
            status: "success",
            description: "USER_LOGOUT_SUCCESSFULY",
            entity_type: "user",
            entity_id: refreshToken.user_id,
            old_data: null,
            new_data: JSON.stringify({
    ...
    async me(userId, client) {
        ...
        await logService.create({
            ...
            action: "USER_AUTH_ME",
            status: "success",
            description: "USER_ACCOUNT_INFO_RETRIEVED",
            entity_type: "user",
            entity_id: user.id,
            old_data: null,
            new_data: JSON.stringify({
    ...
    async forgetPassword({ email, client }) {
        ...
        await logService.create({
            ...
            action: "USER_FORGET_PASSWORD",
            status: "success",
            description: "USER_GET_PASSWORD_RESET_EMAIL",
            entity_type: "user",
            entity_id: user.id,
            old_data: null,
            new_data: JSON.stringify({
    ...
    async confirmEmail({ token, client }) {
        ...
        if (expiresAt <= now) {
            ...
            await logService.create({
                ...
                action: "USER_CONFIRM_EMAIL",
                status: "fail",
                description: "EXPIRED_CONFIRM_TOKEN_TYPE",
                entity_type: "user",
                entity_id: null,
                old_data: null,
                new_data: JSON.stringify({
     ...
     async resendConfirmationEmail({ email, client }) {
        ...
        if (!user || !user.id) {
            await logService.create({
                actor_id: null,
                actor_role: null,
                action: "USER_RESEND_CONFIRM_EMAIL",
                status: "fail",
                description: "USER_NOT_FOUND_BY_EMAIL",
                entity_type: "user",
                entity_id: null,
                old_data: null,
                new_data: JSON.stringify({
        ...
        if (user.email_verified == 1) {
            ...
            await logService.create({
                ...
                action: "USER_RESEND_CONFIRM_EMAIL",
                status: "fail",
                description: "USER_HAS_VERIFY_EMAIL",
                entity_type: "user",
                entity_id: user.id,
                old_data: null,
                new_data: JSON.stringify({
        ...
        await logService.create({
            ...
            action: "USER_RESEND_CONFIRM_EMAIL",
            status: "success",
            description: "USER_VERIFY_EMAIL_SEND",
            entity_type: "user",
            entity_id: user.id,
            old_data: null,
            new_data: JSON.stringify({
