...
describe("TOKEN", () => {
    test("NORMAL", () => {
        ...
        expect(tokenValidation.validate("7cd02c5721e2bccf836720fbe9c83b1bef8aa146cd455fda97237869dec46461fcf494706c6b872379ebcaac7ef013877b6b4f50cda4183498c31cfab553f38c").error).toBeUndefined();
    ...
    test("RANDOM_TEST", ()=>{
        for (let i = 0; i < RANDOM_TEST_LENGTH; i++) {
            if(i % 2){
                expect(tokenValidation.validate(token.generate()).error).toBeUndefined()
            }else{
                expect(tokenValidation.validate(token.generate().slice(Math.random() * 64, (Math.random()*64) + 64)).error.details[0].message).toBe("TOKEN_TOO_SHORT_OR_LONG")
            }
        }
    })
})